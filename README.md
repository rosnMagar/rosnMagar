# HI! I'm Roshan Thapa Magar 👋

I am a Computer and Data Science student at Truman State University with a passion for building scalable applications, AI-driven automation, and utilizing AI to solve problems in medicine and biology. Currently, I serve as the President of the Association for Computing Machinery (ACM) at Truman.

### 🔭 Currently working on
- **OpticAI**: Developing deep learning models for the early detection of diabetic retinopathy.
- **Agentic AI Workflows**: Building CLI-based tools for business automation and intelligent content generation.
- **BulldogRides**: Real-time ride matching for the Kirksville community free of cost.

### 🚀 Featured Projects
- **[BulldogRides](https://github.com/rosnMagar/BulldogRides)**: A full-stack ride-sharing application built with AWS Amplify, GraphQL (AppSync), and DynamoDB.
- **[Movie Editor AI](https://github.com/rosnMagar)**: An automated video generation tool using Python and MoviePy that has powered content reaching over 1 million views.
- **IoT & Robotics**: Experimenting with ESP32-based systems, including emotive robot eye displays using TFT screens.

### 🛠️ Tech Stack
#### **Languages**
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) ![C#](https://img.shields.io/badge/c%23-%23239120.svg?style=for-the-badge&logo=c-sharp&logoColor=white) ![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white) ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E) ![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white) ![SQL](https://img.shields.io/badge/sql-%234479A1.svg?style=for-the-badge&logo=postgresql&logoColor=white)

#### **Web & Cloud**
![React](https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB) ![.Net](https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white) ![AWS](https://img.shields.io/badge/AWS-%23FF9900.svg?style=for-the-badge&logo=amazon-aws&logoColor=white) ![Amplify](https://img.shields.io/badge/AWS%20Amplify-%23FF9900.svg?style=for-the-badge&logo=aws-amplify&logoColor=white) ![DynamoDB](https://img.shields.io/badge/Amazon%20DynamoDB-4053D6?style=for-the-badge&logo=Amazon%20DynamoDB&logoColor=white)

#### **AI & Tools**
![PyTorch](https://img.shields.io/badge/PyTorch-%23EE4C2C.svg?style=for-the-badge&logo=PyTorch&logoColor=white) ![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white) ![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white) ![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white)
### 📊 GitHub Stats
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=rosnMagar&layout=compact&theme=radical)

### 🌱 I’m currently learning
- Advanced Machine Learning architectures and LLM fine-tuning.
- Scaling cloud-native applications and microservices.

<!--
**rosnMagar/rosnMagar** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
